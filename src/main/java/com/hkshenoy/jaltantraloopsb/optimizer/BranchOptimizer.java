...
//	private static boolean removeZeroDemandNodes = false;
	...
	private void addVariablesBase(boolean includePump, boolean includeParallel, boolean includeESR, boolean includeNodeVars) throws Exception {
		for (Pipe pipe : pipes.values()) {
			...
			if (includePump && pumpGeneralProperties.pump_enabled) {
				addPumpVariables(i);
			}

			if (includeParallel && pipe.isAllowParallel()) {
				addParallelPipeVariables(i);
			}

			Node startNode = pipe.getStartNode();
			addHeadVariables(startNode.getNodeID(), i);

			for (int j = 0; j < pipeCost.size(); j++) {
				addPipeCostVariables(i, j, includeParallel);
			}
		}

		if (includeESR) {
			addESRVariables(includeNodeVars);
		}
	}

	private void addPumpVariables(int i) throws Exception {
	...
	private void addParallelPipeVariables(int i) throws Exception {
		problem.setVarType("p_" + i + "_0", Boolean.class);
		problem.setVarType("p_" + i + "_1", Boolean.class);
	}

	private void addHeadVariables(int startNodeId, int i) throws Exception {
		problem.setVarType("head_" + startNodeId + "_" + i, Double.class);
		problem.setVarLowerBound("head_" + startNodeId + "_" + i, 0);

		problem.setVarType("k_" + startNodeId + "_" + i, Boolean.class);

		problem.setVarType("yhead_" + startNodeId + "_" + i, Double.class);
	}

	private void addPipeCostVariables(int i, int j, boolean includeParallel) throws Exception {
	...
	private void addESRVariables(boolean includeNodeVars) throws Exception {
	...
	public void addVariables() throws Exception {
		addVariablesBase(false, false, false, false);
	}

	public void addVariables_gen() throws Exception {
		addVariablesBase(false, false, true, false);
	}

	public void addVariables_gen3() throws Exception {
		addVariablesBase(false, true, true, true);
	}

	public void addVariables_gen4() throws Exception {
		addVariablesBase(true, true, true, true);
	}

	public void addVariables_gen5() throws Exception {
		addVariablesBase(true, true, true, false);
	}
	// Helper method to add pipe costs
	private void addPipeCosts(Linear linear, Pipe pipe, int i, int j, PipeCost entry) {
	...
	// Helper method to add ESR costs
	private void addEsrCosts(Linear linear, int i) {
		int j = 0;
	...
	// Helper method to add pump costs
	private void addPumpCosts(Linear linear, int i) {
	...
	// Helper method to add ESR height factor
	private void addEsrHeightFactor(Linear linear) {
		...
		for (Node node : nodes.values()) {
			linear.add(avgesrcost, "esr_" + node.getNodeID());
		}
	}
	private void setObjectiveCost_esr_gen5() throws Exception {
		setObjectiveCostCommon(true, true, true, true);
	}
	private void setObjectiveCost_esr_gen4() throws Exception {
		setObjectiveCostCommon(true, true, true, false);
	}
	private void setObjectiveCost_esr_gen3() throws Exception {
		setObjectiveCostCommon(true, false, true, false);
	}
	private void setObjectiveCost_esr_gen() throws Exception {
		setObjectiveCostCommon(false, false, true, false);
	}
	private void setObjectiveCost_esr() throws Exception {
		setObjectiveCostCommon(false, false, false, false);
	}
	private void setObjectiveCostCommon(boolean includePumpCosts, boolean includeEsrHeightFactor, boolean includeEsrForAllNodes, boolean includeSiJVariables) throws Exception {
		...
		for (Pipe pipe : pipes.values()) {
			...
			for (PipeCost entry : pipeCost) {
				addPipeCosts(linear, pipe, i, j, entry);
				j++;
			}
			if (includePumpCosts) {
				addPumpCosts(linear, i);
			}
		}
		for (Node node : nodes.values()) {
			int i = node.getNodeID();
			if (includeEsrForAllNodes || node.getDemand() > 0) {
				addEsrCosts(linear, i);
			}
		}
		if (includeEsrHeightFactor) {
			addEsrHeightFactor(linear);
		}
...
//					problem.add(new Constraint("", linear, "=", 1));
	...
	private void addConstraint(String description, Linear linear, String operator, double value) throws Exception {
		problem.add(new Constraint(description, linear, operator, value));
	}
	private void addESRHeightConstraints(Node node, Linear linear) throws Exception {
		int i = node.getNodeID();
	...
	private void addDemandConstraints(Node startNode, Pipe pipe, Linear linear) throws Exception {
		...
		if (startNode.getAllowESR()) {
			...
			addConstraint("", linear, "=", startNode.getElevation());

			// yhead range constraints
			int minEHead = -10000;
			int maxEHead = 10000;

			linear.add(1, "yhead_" + startid + "_" + i);
			addConstraint("", linear, "<=", maxEHead);

			linear.add(1, "yhead_" + startid + "_" + i);
			addConstraint("", linear, ">=", minEHead);
	...
	private void handleParallelPipes(Pipe pipe, Linear linear, int pipeID) {
		int j = 0;
		for (PipeCost entry : pipeCost) {
			...
			linear.add(headlossSecondary, "p_" + pipeID + "_" + j);
			linear.add(headloss - headlossSecondary, "yp_" + pipeID + "_" + j);
			j++;
		}
	}

	private void handleNonParallelPipes(Pipe pipe, Linear linear, int pipeID) {
		int j = 0;
		for (PipeCost entry : pipeCost) {
			...
			linear.add(headlossSecondary, "l_" + pipeID + "_" + j);
			linear.add(headloss - headlossSecondary, "y_" + pipeID + "_" + j);
			j++;
		}
	}

	private void addNodeConstraints(Node node, Linear linear) throws Exception {
		int i = node.getNodeID();
		if (source == node) {
			linear.add(1, "head_" + i);
			addConstraint("", linear, "=", source.getHead());
		} else {
			// ESR height constraints
			linear.add(minEsrHeight, "besr_" + i);
			linear.add(-1, "esr_" + i);
			addConstraint("", linear, "<=", 0);
			...
			linear.add(1, "head_" + i);
			linear.add(-1, "esr_" + i);
			addConstraint("", linear, ">=", node.getElevation() + node.getResidualPressure());
		}
	}
	...
	private void setHeadLossConstraints() throws Exception {
		for (Node node : nodes.values()) {
			Linear linear = new Linear();
			addNodeConstraints(node, linear);

			for (Pipe pipe : node.getSourceToNodePipes()) {
				linear = new Linear();
				int i = pipe.getPipeID();
				if (pipe.isAllowParallel()) {
					handleParallelPipes(pipe, linear, i);
				} else {
					handleNonParallelPipes(pipe, linear, i);
				}
				...
				// Adding the constraint for the head loss
				linear.add(-1, "headloss_" + i);
				addConstraint("", linear, "=", 0);

				// Additional constraints for demand nodes
				if (node.getDemand() > 0) {
					addDemandConstraints(node, pipe, linear);
				} else {
					linear = new Linear();
					linear.add(1, "headloss_" + i);
					linear.add(1, "head_" + pipe.getEndNode().getNodeID());
					linear.add(-1, "head_" + pipe.getStartNode().getNodeID());
					addConstraint("", linear, "=", 0);
				}
			}
		}
		...
		for (Node node : nodes.values()) {
			...
			for (Pipe pipe : pipes.values()) {
				...
				if (startNode.getDemand() > 0) {
					addDemandConstraints(startNode, pipe, linear);
				} else {
					linear = new Linear();
					linear.add(1, "headloss_" + i);
					linear.add(1, "head_" + pipe.getEndNode().getNodeID());
	...
	private void setHeadLossConstraints_esr_gen2() throws Exception {
		...
		for (Node node : nodes.values()) {
			...
			addESRHeightConstraints(node, linear);
		}
	...
	private void setHeadLossConstraints_esr_gen3() throws Exception {
		for (Node node : nodes.values()) {
			...
			for (Pipe pipe : node.getSourceToNodePipes()) {
				int i = pipe.getPipeID();
				linear = new Linear(); // Reset linear for each pipe

				if (pipe.isAllowParallel()) {
					// Handle parallel pipes
					handleParallelPipes(pipe, linear, i);
				} else {
					// Handle non-parallel pipes
					handleNonParallelPipes(pipe, linear, i);
				}
	...
	private void setEsrConstraints_gen() throws Exception {
		for (Node node : nodes.values()) {
			int i = node.getNodeID();
			addOutgoingPipeConstraints(node, i);
			addServiceCoverageConstraint(node, i);
			addDemandConstraint(node, i);

			if (node.getAllowESR()) {
				addEsrConstraints(node, i, false);
			}
		}
	...
	private void addOutgoingPipeConstraints(Node node, int i) throws Exception {
		for (Pipe pipe : node.getOutgoingPipes()) {
	...
	private void addServiceCoverageConstraint(Node node, int i) throws Exception {
		Linear linear = new Linear();
		linear.add(1, "s_" + i + "_" + i);
		for (Node upstreamNode : node.getUpstreamNodes()) {
			int j = upstreamNode.getNodeID();
	...
	private void addDemandConstraint(Node node, int i) throws Exception {
		Linear linear = new Linear();
		linear.add(node.getRequiredCapacity(esrCapacityFactor), "s_" + i + "_" + i);
		for (Node downstreamNode : node.getDownstreamNodes()) {
			int j = downstreamNode.getNodeID();
	...
	private void addEsrConstraints(Node node, int i, boolean includeCapacity) throws Exception {
		int j = 0;
		Linear linearE = new Linear();
		Linear linearZ = new Linear();

		for (EsrCost esr : esrCost) {
			if (includeCapacity) {
				addEsrCapacityConstraints(i, j, esr);
			}

			linearE.add(1, "e_" + i + "_" + j);
			linearZ.add(1, "z_" + i + "_" + j);
			j++;
		}

		problem.add(new Constraint("", linearE, "=", 1));
		linearZ.add(-1, "d_" + i);
		problem.add(new Constraint("", linearZ, "=", 0));
	}

	private void addEsrCapacityConstraints(int i, int j, EsrCost esr) throws Exception {
		Linear linear = new Linear();
		linear.add(esr.getMinCapacity(), "e_" + i + "_" + j);
		linear.add(-1, "z_" + i + "_" + j);
		problem.add(new Constraint("", linear, "<=", 0));
	...
	private void addPipeConstraints() throws Exception {
		for (Pipe pipe : pipes.values()) {
	...
	private void setEsrPipeConstraints() throws Exception {
		for (Pipe pipe : pipes.values()) {
			createFlowConstraint(pipe, false);
		}
		for (Pipe pipe : pipes.values()) {
			applyPipeConstraints(pipe);
		}
		enforceChildPipesConsistency();
		validateSourceToNodePipeConstraints();
	}
	// Set constraints relating ESR to pipe network (with zero demand nodes)
	...
	// Set constraints relating ESR to pipe network (some constraints removed)
	private void setEsrPipeConstraints_gen3() throws Exception {
		for (Pipe pipe : pipes.values()) {
			createFlowConstraint(pipe, false);
		}
	}
	// Set constraints relating ESR to pipe network (with updated variables)
	private void setEsrPipeConstraints_gen4() throws Exception {
		for (Pipe pipe : pipes.values()) {
			createFlowConstraint(pipe, true);
		}
		for (Pipe pipe : source.getOutgoingPipes()) {
			applySourceConstraints(pipe);
		}
	}
	// Helper method to create flow constraints
	private void createFlowConstraint(Pipe pipe, boolean useBesr) throws Exception {
		Linear linear = new Linear();
		int i = pipe.getPipeID();
		...
		linear.add(1, "f_" + i);
		int j = end.getNodeID();
		linear.add(-1, (useBesr ? "besr_" : "s_") + j + "_" + j);

		problem.add(new Constraint("", linear, "=", 0));
	}
	// Helper method to apply pipe constraints
	private void applyPipeConstraints(Pipe pipe) throws Exception {
		int i = pipe.getPipeID();

		for (int j = 0; j < pipeCost.size(); j++) {
			addLengthConstraints(pipe, i, j);
			if (pipe.isAllowParallel()) {
				addParallelConstraints(pipe, i, j);
			}
		}
		if (pipe.isAllowParallel()) {
			addParallelConstraintsForPipe(pipe, i);
		}
	}
	// Helper method for length-related constraints
	private void addLengthConstraints(Pipe pipe, int i, int j) throws Exception {
		Linear linear;
	...
	// Helper method for parallel-related constraints
	private void addParallelConstraints(Pipe pipe, int i, int j) throws Exception {
		Linear linear;
	...
	// Helper method for parallel constraints for pipe
	private void addParallelConstraintsForPipe(Pipe pipe, int i) throws Exception {
		Linear linear;
	...
	// Helper method to enforce consistency among child pipes
	private void enforceChildPipesConsistency() throws Exception {
		for (Node node : nodes.values()) {
			if (node.getDemand() == 0) {
				List<Pipe> childPipes = node.getOutgoingPipes();
				if (!childPipes.isEmpty()) {
	...
	// Helper method to validate constraints for source to node pipes
	private void validateSourceToNodePipeConstraints() throws Exception {
		for (Node node : nodes.values()) {
			if (node.getDemand() > 0) {
	...
	// Helper method to apply constraints for source pipes
	private void applySourceConstraints(Pipe pipe) throws Exception {
		Linear linear = new Linear();
		int i = pipe.getPipeID();
	...
	// Helper function to handle pump calculations and settings
	private void handlePumpCosts(Pipe pipe, Result result, int i) throws Exception {
		if (pumpGeneralProperties.pump_enabled) {
			double pumphead = Util.round(result.getPrimalValue("pumphead_" + i).doubleValue(), 5);
			double primarypower = Util.round(result.getPrimalValue("ppumppower_" + i).doubleValue(), 5);
			double secondarypower = Util.round(result.getPrimalValue("spumppower_" + i).doubleValue(), 5);
			double power = Util.round(result.getPrimalValue("pumppower_" + i).doubleValue(), 5);
	...
	// Helper function to handle pipe dimensions
	private void handlePipeDimensions(Pipe pipe, Result result, int i, boolean isExistingPipe, boolean isGen4) throws Exception {
		int noOfSubPipes = 0;
		int j = 0;
		for (PipeCost entry : pipeCost) {
			double length1 = Util.round(result.getPrimalValue("l_" + i + "_" + j + "_0").doubleValue(), 5);
			double length2 = Util.round(result.getPrimalValue("l_" + i + "_" + j + "_1").doubleValue(), 5);
			...
			if (isExistingPipe) {
				...
				if (entry.getDiameter() == pipe.getDiameter() && length != pipe.getLength())
	...
	// Main function to set diameters from result
	private void setDiametersFromResult(Result result) throws Exception {
		for (Pipe pipe : pipes.values()) {
			...
			handlePumpCosts(pipe, result, i);

			boolean isExistingPipe = pipe.existingPipe();
			boolean isGen4 = true; // Assuming gen4 logic as it handles pumps as well
			handlePipeDimensions(pipe, result, i, isExistingPipe, isGen4);
		}
	}
	// Specialized function for Gen3 scenario
	private void setDiametersFromResult_gen3(Result result) throws Exception {
		for (Pipe pipe : pipes.values()) {
			int i = pipe.getPipeID();
			int flowChoice = result.getPrimalValue("f_" + i).intValue();
			pipe.setFlowchoice(flowChoice == 1 ? FlowType.PRIMARY : FlowType.SECONDARY);
			handlePumpCosts(pipe, result, i); // No pump handling in Gen3
			boolean isExistingPipe = pipe.existingPipe();
			boolean isGen4 = false; // Gen3 doesn't have pump handling
			handlePipeDimensions(pipe, result, i, isExistingPipe, isGen4);
		}
	}
	// Specialized function for Gen4 scenario
	private void setDiametersFromResult_gen4(Result result) throws Exception {
		for (Pipe pipe : pipes.values()) {
			...
			handlePumpCosts(pipe, result, i); // Gen4 has pump handling

			boolean isExistingPipe = pipe.existingPipe();
			boolean isGen4 = true; // Gen4 specific logic
			handlePipeDimensions(pipe, result, i, isExistingPipe, isGen4);
		}
	}
